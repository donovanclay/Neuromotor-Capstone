...
class Processing_HUB(QObject):
    ...
    def __init__(self):
        ...
        self.controller.Display1.connect(self.handle_Display1)
        self.controller.Display2.connect(self.handle_Display2)
        self.pause_signal.connect(self.controller.pause)
        self.resume_signal.connect(self.controller.resume)
...
# Below functions are for data selection
    ...
    def data_selection(self): 
        ...
        if self.data_selected in [0,1]:
            self.eeg_array=F4G.Eeg_data_function(self.subject, self.trials, [2,17],drop_excluded=False)
            self.eeg_raw=self.eeg_array.isel(Trial=0).copy()
            self.Eog_filter()
    ...
    def Eog_filter(self):
        self.mne_eeg={}
        self.weights={}
        self.ch_names, self.xyz=F4G.remap_spatial_2_channels(Include_none=True)
        self.ch_pos= {key: coord for key, coord in zip(self.ch_names, self.xyz)}
        names=F4G.get_included_chan_names(drop_excluded=False)
        for trial in self.trials:
            raw_clean, cleaned_data, weights = F4G.EoG_filter(self.eeg_array.isel(Trial=trial-1), names, self.ch_pos)
            self.mne_eeg[trial] = raw_clean
            self.eeg_array[trial-1] = cleaned_data
            self.weights[trial] = weights
...
### Functions below handle perminant changes to the EEG data
    ...
    def generate_parameter_array(self):
        ...
        if self.current_index==3 and hasattr(self, 'raw'):
             self.parameters.append(self.raw)  
...
### Functions below are responsible for interactive graphing changes
    ...
    def update_position_data(self):
         if self.first_graph1 == 0:
            if len(self.ch_names) !=60:
                EoG_indx=F4G.get_EoG_index(self.ch_names)
                self.ch_names = np.delete(self.ch_names, EoG_indx)
            if len(self.xyz) != 60:
                EoG_indx = F4G.get_EoG_index()
                self.xyz = np.delete(self.xyz, EoG_indx, axis=0)
            if self.eeg_raw.shape[1] !=60:
                self.eeg_raw=F4G.apply_function_by_trial(self.eeg_raw, function=F4G.drop_eog_channels)
            self.ch_pos={key: coord for key, coord in zip(self.ch_names, self.xyz)}
            self.first_graph1+=1
    ...
    def plot_weights(self):
        ...
        axs = axs.ravel()  
        trial_key = self.trials[0]
        if trial_key in self.weights:
            self.weights[trial_key].plot(axes=axs)
        else:
            print(f"Trial {trial_key} or weight index 0 not found in self.weights")
        plt.suptitle('Weights fit by linear regression to remove EoG artifacts', fontsize=16)
        fig.canvas.draw()
    ...
    def PCA_chooser(self, *args):
        if args:  # Check if any positional arguments are passed
            eeg_data = args[0]  # Assuming the first positional argument is the EEG data
        else:
            eeg_data = self.eeg_previous
        
        self.pca = PCA(n_components=self.slider_value, random_state=1)
        self.pca.fit(eeg_data)
        self.pca_trans = self.pca.transform(eeg_data)
    ...
    def ICA_chooser(self):
        if self.first_graph2 == 0:
            self.raw = F4G.mne_data_type_converter(self.eeg_previous, self.ch_names.tolist(), 'eeg')
            montage=F4G.get_montage(self.ch_pos)
            self.raw.set_montage(montage)
            self.ica = mne.preprocessing.ICA(n_components=self.ica_comp, random_state=1, max_iter=800)
            self.ica.fit(self.raw)
            self.sources = self.ica.get_sources(self.raw).get_data()
            muscle_idx_auto, scores = self.ica.find_bads_muscle(self.raw)
            F4G.plot_components(self.eeg_previous, self.sources, muscle_idx_auto)  # can be silenced if you don't want figure
            print('The components being dropped are', muscle_idx_auto)
            self.first_graph2+=1
        value=self.slider_value
        self.ica = mne.preprocessing.ICA(n_components=self.ica_comp, random_state=1, max_iter=800)
        self.ica.fit(self.raw)
        data_reconstructed = self.ica.apply(self.raw.copy(), exclude=muscle_idx_auto,n_pca_components=value) # Need to double check working as intended
        self.eeg_filt = np.transpose(data_reconstructed.get_data())
        self.eeg_filtered = self.eeg_filt
        self.variance()
        self.graph_components()
    ...
    def graph_signals(self):
        ...
        fig, ax = plt.subplots()
        if self.current_index==1:
            plt.plot(self.eeg_previous.Recording.Time, self.eeg_raw.isel(Channel=12), label='Original Signal', alpha=0.7)
    ...
    def play_eeg(self):
        if self.play_eeg_instance is not None:
            if self.play_eeg_instance.isRunning():
        ...
        if self.eeg_previous is not None and len(self.eeg_previous) > 0:
            self.play_eeg_instance = DisplayVideo1(self.eeg_raw, self.eeg_filtered, self.ch_names, self.xyz, self)
            self.play_eeg_instance.Display1.connect(self.handle_Display1)
            self.play_eeg_instance.Display2.connect(self.handle_Display2)
...
### Following functions deal with applying functions to entire dataset one channel at a time
        ...
        def apply_self_contained(self):
            if self.function_index == 1:
                eeg_data=F4G.apply_function_by_trial(self.eeg_data, function=F4G.drop_eog_channels)
                channels=eeg_data.Channel
                eeg_data = F4G.apply_function_by_channel(eeg_data, function=F4G.normalize_eeg,channels=channels)
                self.eeg_data = F4G.apply_function_by_channel(eeg_data,function=F4G.remove_artifacts,channels=channels)
...
### Following functions deals with calling and selecting specific functions to be applied to dataset
        ...
        def current_function(self, data):
            ...
            if self.function_index==5:
                 self.remove_EoG_channels(data)
        ...
        def remove_EoG_channels(self,data):
            indices_to_drop = F4G.get_EoG_index()
            data = data.drop_sel(Channel=indices_to_drop)
        ...
        def ica_wrapper(self):
            self.ica = mne.preprocessing.ICA(n_components=30, random_state=1, max_iter=800)
            self.ica.fit(self.raw)
            muscle_idx_auto, scores = self.ica.find_bads_muscle(self.raw)
            data=self.ica.apply(self.raw.copy(), exclude=muscle_idx_auto, n_pca_components=self.components).get_data()  # seeing if excluding components helps
            return self.eeg_data            
...
class DisplayVideo1(QThread):
    request_start_timer = pyqtSignal()
    request_stop_timer = pyqtSignal()
    Display1 = pyqtSignal(QImage)
    Display2 = pyqtSignal(QImage)

    def __init__(self, raw_data, eeg_data, channel_name, channel_coords, hub, **kwargs):
        super(DisplayVideo1, self).__init__()
        self.eeg_data = eeg_data
        ...
        self.lag = 80  # milliseconds needed for syncing with movement but not used yet
        self.ch_type = None
        channels=self.raw_data.Channel
        self.raw_data = F4G.apply_function_by_channel2(self.raw_data, function=F4G.normalize_eeg,channels=channels)
        self.raw_data=self.raw_data[0]
        raw_eeg = self.raw_data.mean(axis=1)
        self.raw_eeg_average = raw_eeg.values
        self.average_eeg = np.mean(eeg_data, axis=1)
        self.frame_index = self.vid_frame_rate
        self.time = range(len(self.eeg_data))
        self.hub = hub
    ...
    def get_video(self):
        if self.eeg_data is not None and self.ThreadActive:
            if self.frame_index < self.raw_data.values.shape[0] - self.vid_frame_rate:
                if not self.stop_thread:
                    image_data = self.eeg_image(self.eeg_data, self.frame_index, 0)
                    raw_array = self.eeg_image(self.raw_data, self.frame_index, 1)
                    image_array = np.hstack((raw_array, image_data))
                    image_2 = self.eeg_graph(self.average_eeg, self.raw_eeg_average,self.frame_index)
                    if image_array is not None:
                        height, width, channel = image_array.shape
                        img_bytes = image_array.tobytes()
                        q_img = QImage(img_bytes, width, height, channel * width, QImage.Format_RGB888)
                        ...
                        print('Failed to get image_array frame')
                    if image_2 is not None:
                        height2, width2, channel2 = image_2.shape
                        img_bytes2 = image_2.tobytes()
                        q_img2 = QImage(img_bytes2, width2, height2, channel2 * width2, QImage.Format_RGB888)
                        self.Display2.emit(q_img2)
                    else:
                        print('Failed to get image_2 frame')
                else:
    ...
    def eeg_image(self, data, index_point, text_swapper=0):
        ...
        image_array = np.frombuffer(buf, np.uint8).reshape(height, width, 4)
        image_array = image_array[:, :, :3]
        plt.close(fig)
        return image_array

    def eeg_graph(self,data,raw,index_point):
            matplotlib.use('Agg')
            upper_bound = index_point + self.average_half
            lower_bound = index_point - self.average_half
            if lower_bound < 0 or upper_bound > len(self.time):
                print(f"Bounds out of range: lower_bound={lower_bound}, upper_bound={upper_bound}")
                return None
            fig, ax = plt.subplots(figsize=(10, 6))
            plt.plot(self.time[lower_bound:upper_bound], raw[lower_bound:upper_bound], label='Original Signal', alpha=0.7)
            plt.plot(self.time[lower_bound:upper_bound], data[lower_bound:upper_bound], label='Filtered Signal', alpha=0.7)
            plt.title('Average Signal')
            plt.xlabel('Time (ms)')
            plt.ylabel('Amplitude')
            plt.xlim([self.time[lower_bound], self.time[upper_bound]])
...
class MainWindow(QDialog):
    def __init__(self):
        ...
        self.Max_slider = [[0],[10,10,50],[40],[0],[0],[0]]
 ...
 # Stacked Widget for switching between control sets
    ...
    def init_new_widget(self):
            ...
            self.save_data_page()
            self.transform_page()
            self.Button_manager=[[self.comboBox1,self.Box_label, self.comboBox2,self.Box_label2],[0],[self.comboBoxca],[0],[0], [0]] # , self.Box_labelca
    ...
    def component_page(self):
        ...
        # self.Box_labelca.setVisible(True)
        self.comboBoxca.currentTextChanged.connect(self.sel_method)
        self.comboBoxca.setVisible(True)
    ...
    def visual_denoising_verification(self):
            ...
            self.pause_button = QPushButton(self.page_CA_verification, text="Pause")
            self.pause_button.setGeometry(900, 10, 200, 60)
            self.resume_button = QPushButton(self.page_CA_verification, text="Resume")
            self.resume_button.setGeometry(600, 10, 200, 60)
            self.pause_button.clicked.connect(self.pause_signal)
            self.resume_button.clicked.connect(self.resume_signal)
