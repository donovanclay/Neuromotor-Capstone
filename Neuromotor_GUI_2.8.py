...
import numpy as np
import functions_4_GUI4 as F4G
...
### Functions below are responsible for interactive graphing changes
    ...
    def ICA_chooser(self):
        if self.first_graph1 == 0:
            ...
            self.ica = mne.preprocessing.ICA(n_components=20, random_state=1, max_iter=800)
            self.ica.fit(self.raw)
            self.sources = self.ica.get_sources(self.raw).get_data()
            F4G.plot_components(self.eeg_previous, self.sources)  # can be silenced if you don't want figure
        value=self.slider_value
        self.ica = mne.preprocessing.ICA(n_components=value, random_state=1, max_iter=800)
        self.ica.fit(self.raw)
        self.ica.exclude = [1, 2]
        if value >= 10:
            self.ica.exclude.append(10)
        if value >= 13:
            self.ica.exclude.append(13)
        data_reconstructed = self.ica.apply(self.raw.copy(), exclude=self.ica.exclude) # Need to double check working as intended
...
### Following functions deals with calling and selecting specific functions to be applied to dataset
        ...
        def ica_wrapper(self,data):
            ...
            if value >= 13:
                self.ica.exclude.append(13)
            data= self.ica.apply(self.raw.copy(), exclude=self.ica.exclude)  # seeing if excluding components helps
